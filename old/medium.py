# //////////////////////////////////////////////// //
#                                                  //
# python3 script to read the medium levelmeter     //
#                                                  //
...
if determine_baseline:
    headerFile = open('medium_'+today+'.txt', 'a')
...
baseline = 157.408 #157.427 # 159.3255804444 # 158.780272719 # capacitance [pF] when the levelmeter is in GAr only and the temperature is ~ the operation temperature
dC_dx = 0.05 # pF/mm
#dist_bottom_top_flange = 1610 # [mm] for the prototype module
...
while counter < measurements or measurements == 0:
    ...
    while ser.inWaiting() > 1:
        ...
        if determine_baseline:
            saveFile = open('medium_'+today+'.txt', 'a') # for appending: 'a'; directory: 'c:/.../.../data.txt'
        ...
        if second_old == second_new:
            ...
            post_1s_pF = "lvl_pF,size=medium,pos=module value=" + str(c+mean_of_1second-baseline)
            post_1s_mm = "lvl_mm,size=medium,pos=module value=" + str((c+mean_of_1second-baseline)/dC_dx)
            #print(post_1s_pF)
            #print(baseline)
            #print(post_1s_mm)
            ...
            # Build the mean value of 5 seconds and write it in mean-of-5seconds.txt
                ...
                #post_5s_pF = "lvl_pF,no=0,size=medium,pos=module value=" + str(c+mean_of_5seconds-baseline)
                #post_5s_mm = "lvl_mm,no=0,size=medium,pos=module value=" + str((c+mean_of_5seconds-baseline)/dC_dx) # - dist_bottom_top_flange)
                #print(post_5s_pF)
                #print(baseline)
                #print(post_5s_mm)
